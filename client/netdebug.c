...
static struct dentry *pmnet_dentry;

static DEFINE_SPINLOCK(pmnet_debug_lock);
...
{
	spin_lock(&pmnet_debug_lock);
	list_add(&nst->st_net_debug_item, &send_tracking);
	spin_unlock(&pmnet_debug_lock);
}
...
static struct pmnet_send_tracking
			*next_nst(struct pmnet_send_tracking *nst_start)
{
	struct pmnet_send_tracking *nst, *ret = NULL;

	assert_spin_locked(&pmnet_debug_lock);
...
{
	struct pmnet_send_tracking *nst, *dummy_nst = seq->private;
...
{
	struct pmnet_send_tracking *dummy_nst;

	dummy_nst = __seq_open_private(file, &nst_seq_ops, sizeof(*dummy_nst));
	if (!dummy_nst)
		return -ENOMEM;
	pmnet_debug_add_nst(dummy_nst);
...
static int nst_fop_release(struct inode *inode, struct file *file)
{
	...
	struct pmnet_send_tracking *dummy_nst = seq->private;

	pmnet_debug_del_nst(dummy_nst);
...
struct pmnet_sock_debug {
	int dbg_ctxt;
	struct pmnet_sock_container *dbg_sock;
};

static struct pmnet_sock_container
			*next_sc(struct pmnet_sock_container *sc_start)
{
...
{
	struct pmnet_sock_debug *sd = seq->private;
	struct pmnet_sock_container *sc, *dummy_sc = sd->dbg_sock;
...
/* So that debugfs.pmdfc can determine which format is being used */
#define PMNET_STATS_STR_VERSION		1
...
{
	struct pmnet_sock_debug *sd;
	struct pmnet_sock_container *dummy_sc;
...
static int sc_fop_release(struct inode *inode, struct file *file)
{
	struct seq_file *seq = file->private_data;
	struct pmnet_sock_debug *sd = seq->private;
	struct pmnet_sock_container *dummy_sc = sd->dbg_sock;

	pmnet_debug_del_sc(dummy_sc);
	kfree(dummy_sc);
	return seq_release_private(inode, file);
}
...
static int pmnet_fill_bitmap(char *buf, int len)
{
	unsigned long map[BITS_TO_LONGS(PMNM_MAX_NODES)];
	int i = -1, out = 0;

	pmnet_fill_node_map(map, sizeof(map));

	while ((i = find_next_bit(map, PMNM_MAX_NODES, i + 1)) < PMNM_MAX_NODES)
...
static int nodes_fop_open(struct inode *inode, struct file *file)
{
	...
	i_size_write(inode, pmnet_fill_bitmap(buf, PAGE_SIZE));
...
static int pmnet_debug_release(struct inode *inode, struct file *file)
{
	kfree(file->private_data);
	return 0;
}

static ssize_t pmnet_debug_read(struct file *file, char __user *buf,
...
static const struct file_operations nodes_fops = {
	.open		= nodes_fop_open,
	.release	= pmnet_debug_release,
	.read		= pmnet_debug_read,
	.llseek		= generic_file_llseek,
};

void pmnet_debugfs_exit(void)
{
	debugfs_remove_recursive(pmnet_dentry);
}
...
void pmnet_debugfs_init(void)
{
	...
	pmnet_dentry = debugfs_create_dir(PMNET_DEBUG_DIR, NULL);

	debugfs_create_file(NST_DEBUG_NAME, mode, pmnet_dentry, NULL,
			    &nst_seq_fops);
	debugfs_create_file(SC_DEBUG_NAME, mode, pmnet_dentry, NULL,
			    &sc_seq_fops);
	debugfs_create_file(STATS_DEBUG_NAME, mode, pmnet_dentry, NULL,
			    &stats_seq_fops);
	debugfs_create_file(NODES_DEBUG_NAME, mode, pmnet_dentry, NULL,
			    &nodes_fops);
}
