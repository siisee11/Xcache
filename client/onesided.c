...
int rdpma_put_onesided(struct page *page, u64 roffset)
{
  int ret;
  struct rdma_queue *q;
...
int pmdfc_rdma_read_async(struct page *page, u64 roffset)
{
  ...
  q = rdpma_get_queue(smp_processor_id(), QP_READ_ASYNC);
...
int rdpma_get_onesided(struct page *page, u64 roffset)
{
  struct rdma_queue *q;
  int ret;
...
{
  struct rdma_queue *q = rdpma_get_queue(cpu, QP_READ_SYNC);
