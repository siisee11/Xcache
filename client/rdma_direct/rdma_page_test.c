...
atomic_long_t mr_free_start = ATOMIC_LONG_INIT(0);
...
int rdpma_write_message_test(void* arg){
	...
	for (i = 0; i < ITERATIONS; i++) {
        ...
        item = (struct ht_data *)kmalloc(sizeof(struct ht_data), GFP_ATOMIC);
        item->longkey = get_longkey(key, index);
        item->roffset = atomic_long_fetch_add_unless(&mr_free_start, PAGE_SIZE, mr_free_end);
        if (item->roffset >= mr_free_end) {
            pr_warn("[ WARN ] Remote memory is full..\n");
        }
        hash_add(hash_head, &item->h_node, item->longkey);
		ret = pmdfc_rdma_write(vpages[tid][i], item->roffset);
	}
...
int rdpma_read_message_test(void* arg)
{
	...
	for (i = 0; i < ITERATIONS; i++) {
        ...
        if (atomic_read(&found)) {
            ret = pmdfc_rdma_read_sync(return_page[tid], roffset);
            if(memcmp(page_address(return_page[tid]), page_address(vpages[tid][i]), PAGE_SIZE) != 0) {
                printk("failed Searching for key %u\n return: %s\nexpect: %s\n", 
                        key, (char *)page_address(return_page[tid]), (char *)page_address(vpages[tid][i]));
                nfailed++;
            }
        } else 
            nfailed++;
