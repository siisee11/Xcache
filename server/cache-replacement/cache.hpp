...
class fixed_sized_cache
{
  public:
    ...
    void Put(const Key &key, const Value &value)
    {
        ...
        if (elem_it == cache_items_map.end())
        {
            ...
            if (cache_items_map.size() + 1 > max_cache_size)
            {
                ...
                evict_queue.emplace_front(disp_candidate_key);
    ...
    const void PrintCache() const 
    {
        operation_guard lock{safe_op};
        cache_policy.PrintCache();
    }
    
    const void PrintEvictQueue() const 
    {
        operation_guard lock{safe_op};
        std::cout << "PrintEvictQueue()" << std::endl;
        if (evict_queue.empty()) {
            std::cout << "EvictQueue is empty" << std::endl;
        }
        else {
            for (auto const &i : evict_queue) {
                std::cout << i << " ";
            }
            std::cout << std::endl;
        }
    ...
    const void ClearEvictQueue()
    {
        operation_guard lock{safe_op};
        std::cout << "ClearEvictQueue()" << std::endl;
        evict_queue.clear();
    }
