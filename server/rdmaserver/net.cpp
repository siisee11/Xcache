...
int rdma_write(int node_id, int type, uint64_t addr, int len){
	...
	wr.num_sge = 1;
	wr.send_flags = IBV_SEND_SIGNALED;
	wr.wr.rdma.remote_addr = ctx->remote_mm[node_id] + REQUEST_SIZE; /*writing to reply region*/
	wr.wr.rdma.rkey = ctx->rkey[node_id];
	...
	while(1){
		...
		if((int)wc.opcode == IBV_WC_RECV_RDMA_WITH_IMM){
			...
			if(type == MSG_WRITE_REQUEST){
				...
				struct request_struct* new_request = (struct request_struct*)malloc(sizeof(struct request_struct));
				uint64_t* offset = (uint64_t*)GET_CLIENT_META_REGION(ctx->local_mm, node_id, pid);
				for(int i=0; i<num; i++){
					new_request->keys[i] = *(offset + i*REQUEST_SIZE);
				}
				new_request->type = type;
				new_request->node_id = node_id;
				new_request->pid = pid;
...
void event_handler(void){
	...
	while(1){
		...
		if(new_request->type == MSG_WRITE_REQUEST){
			...
			//void* ptr = mmap(NULL, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, 0, 0);
			//void* ptr = mmap(page, PAGE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, 0, 0);
			...
			uint64_t offset = NUM_ENTRY * REQUEST_SIZE * new_request->pid;
			uint64_t* addr = (uint64_t*)GET_CLIENT_META_REGION(ctx->local_mm, new_request->node_id, new_request->pid);
			*addr = (uint64_t)page;
			//*addr = (uint64_t)&ptr;
			...
			//	    fprintf(stderr, "Processing [MSG_WRITE] %d num pages from node %d with %d pid\n", new_request->num, new_request->node_id, new_request->pid);
			...
			for(int i=0; i<new_request->num; i++){
				uint64_t key;
				memcpy(&key, (ptr+i*PAGE_SIZE), sizeof(uint64_t));
				char* temp = (char*)malloc(sizeof(char)*PAGE_SIZE);
				memcpy(temp, (ptr+i*PAGE_SIZE), PAGE_SIZE);
				dprintf("Copied: %d\n", insert_cnt);

				hashtable->Insert(key, (const char*)temp);
				//hashtable->Insert(keys[i], (const char*)(ptr+i*PAGE_SIZE));
				//dprintf("Inserted Key[%llu] with value\n", key);
				//insert_cnt++;
			}
			/*
			...
			free(ptr);
			//munmap(ptr, new_request->num*PAGE_SIZE);
			...
			bool abort = false;
			for(int i=0; i<new_request->num; i++){
				dprintf("Target Key is %llu\n", new_request->keys[i]);
				values[i] = (void*)hashtable->Get(new_request->keys[i]);
				//search_cnt++;
				if(values[i]){
					//dprintf("Value for key[%llu] found\n", new_request->keys[i]);
					//dprintf("Value is : %s\n", (char*)values[i]);
				}
				else{
...
static struct server_context* server_init_ctx(struct ibv_device* dev, int size, int rx_depth){
	...
	ctx->hashtable = new CCEH(1024*16);
...
int establish_conn(void){
	...
	while(running){
		...
		//fd = accept(sock, (struct sockaddr*)&remote_sock, (void*)&sin_size);
		if(fd < 0){
...
int server_init_interface(){
	...
	/*
	   ret = establish_conn();
	   if(ret){
	   fprintf(stderr, "establish_conn failed\n");
	   return 1;
	   }*/
	pthread_create(&connection_thread, NULL, (void*)&establish_conn, NULL);
	pthread_create(&thread_poll_cq, NULL, (void*)&server_poll_cq, ctx->recv_cq);
	pthread_create(&event_handler_thread, NULL, (void*)&event_handler, NULL);
...
void init_server(){
	...
	//    test_func2();
	//    printf("completed test_function\n");
	/*    struct sigaction sa;
