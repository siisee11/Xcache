...
void CCEH::Insert_extent(PMEMobjpool* pop, uint64_t key ,uint64_t len, Value_t value){
	printf("Key:%ld, len:%ld\n", key, len);
	if(len == 0) return;
	uint64_t head = key;
	if(len == 1){
		Insert(pop, key, value);
		Insert_extent(pop, key+1, len-1, value);
		return;
	}
	...
	while(cover_range > len)
		cover_range >>=1;
	Insert(pop, key, value);
	Insert_extent(pop,  head+cover_range, len-cover_range, value);
	return;
} 
